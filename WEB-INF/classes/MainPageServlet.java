...
public class MainPageServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        String createSessionButton =
            ...
            "  fetch('createsession', { method: 'POST' })" +
            "  .then(res => {" +
            "     if(res.ok) {" +
            "         document.getElementById('sessionStatus').innerText = 'Session created!';" +
            "         location.reload();" +
            "     } else {" +
            "         document.getElementById('sessionStatus').innerText = 'Failed to create session';" +
            "     }" +
            "  })" +
        ...
        html = html.replace("<!-- DISPLAY_NAME -->", displayName)
                   ...
                   .replace("<!-- CREATE_SESSION_BUTTON -->", createSessionButton);
